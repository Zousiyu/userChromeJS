...
if (location.href.indexOf('about:stylish-edit') == 0) {
    ...
    // 3.注释按钮(ctrl+/)
    // 4.插入链结:检测当前打开的窗口和分页列出其地址链结;
    //		左键菜单项直接插入对应的链结;
    //		中键或右键则插入包含@-moz-document url("")的链结
    // 5.插入文本:第一个子菜单为文档规则,其余为一些常用的文本 ，第二个子菜单为备用的插入文本2 可将一些不常用的文本整理至这里
    // 6.显示行和列，在行输入框内输入正整数回车可跳转之对应行
    */
    var stylishCustom2 = {

        _revertOldFindbar: false, //还原旧寻找列(缺省关闭，若开启请将 false , 改为 true);
        AutoExternalEditor: false, //自动使用外部编辑开启(缺省关闭，若开启请将 false , 改为 true);
        ...
        insertRules: {
            ...
            text: [
                ...
                ['::after 伪元素', '::after {\n\tcontent: ""\n}', 3], // 参数 0：菜单名，
                // 参数 1：插入内容，其中\n代表换行符，\t为制表符（tab）。
                // 参数 2：插入内容后光标所在对应内容倒数字置，可忽略缺省为0，即最末
            ],
            text2: [
                '/* AGENT_SHEET */',
                '-moz-box-ordinal-group:',
    ...
    (function winhook(aWindow) {
        ...
        var _localeText = {
            ExternalEditor: ['Editor', '外部编辑', '外部编辑'],
            ColorPicker: ['Color Picker', '选色器', '选色器']
        };
